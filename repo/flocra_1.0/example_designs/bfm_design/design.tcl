proc create_ipi_design { offsetfile design_name } {
	...
	# Create instance: flocra_0, and set properties
	set flocra_0 [ create_bd_cell -type ip -vlnv user.org:user:flocra:1.0 flocra_0]
	...
	connect_bd_intf_net [get_bd_intf_pins master_0/M_AXI ] [get_bd_intf_pins flocra_0/S00_AXI]

	# Create port connections
	connect_bd_net -net aclk_net [get_bd_ports ACLK] [get_bd_pins master_0/ACLK] [get_bd_pins flocra_0/S00_AXI_ACLK]
	connect_bd_net -net aresetn_net [get_bd_ports ARESETN] [get_bd_pins master_0/ARESETN] [get_bd_pins flocra_0/S00_AXI_ARESETN]
...
connect_bd_intf_net [get_bd_intf_pins slave_0/S_AXI ] [get_bd_intf_pins flocra_0/M00_AXI]
	# Create port connections
	connect_bd_net -net aclk_net [get_bd_ports ACLK] [get_bd_pins slave_0/ACLK] [get_bd_pins flocra_0/M00_AXI_ACLK]
	connect_bd_net -net aresetn_net [get_bd_ports ARESETN] [get_bd_pins slave_0/ARESETN] [get_bd_pins flocra_0/M00_AXI_ARESETN]
	connect_bd_net -net init_axi_txn_00 [get_bd_ports M00_AXI_INIT_AXI_TXN] [get_bd_pins flocra_0/M00_AXI_INIT_AXI_TXN]
	connect_bd_net -net error_00 [get_bd_ports M00_AXI_ERROR] [get_bd_pins flocra_0/M00_AXI_ERROR]
	connect_bd_net -net txn_done_00 [get_bd_ports M00_AXI_TXN_DONE] [get_bd_pins flocra_0/M00_AXI_TXN_DONE]
	...
	# Create interface connections
	connect_bd_intf_net [get_bd_intf_pins master_1/M_AXI ] [get_bd_intf_pins flocra_0/S_AXI_INTR]

	# Create port connections
	connect_bd_net -net aclk_net [get_bd_ports ACLK] [get_bd_pins master_1/ACLK] [get_bd_pins flocra_0/S_AXI_INTR_ACLK]
	connect_bd_net -net aresetn_net [get_bd_ports ARESETN] [get_bd_pins master_1/ARESETN] [get_bd_pins flocra_0/S_AXI_INTR_ARESETN]
	set S_AXI_INTR_IRQ [ create_bd_port -dir O -type intr irq ]
	connect_bd_net [get_bd_pins /flocra_0/irq] ${S_AXI_INTR_IRQ}
...
set_property -name {xsim.simulate.runtime} -value {100ms} -objects [get_filesets sim_1]
	...
	set offset_file "${bd_path}/flocra_v1_0_tb_include.svh"
	set fp [open $offset_file "w"]
	puts $fp "`ifndef flocra_v1_0_tb_include_vh_"
	puts $fp "`define flocra_v1_0_tb_include_vh_\n"
	...
	set param_irq_active_state [get_property CONFIG.C_IRQ_ACTIVE_STATE [get_bd_cells flocra_0]]
	set param_irq_sensitivity [get_property CONFIG.C_IRQ_SENSITIVITY [get_bd_cells flocra_0]]
	set param_intr_active_state [get_property CONFIG.C_INTR_ACTIVE_STATE [get_bd_cells flocra_0]]
	set param_intr_sensitivity [get_property CONFIG.C_INTR_SENSITIVITY [get_bd_cells flocra_0]]
...
set ip_path [file dirname [file normalize [get_property XML_FILE_NAME [ipx::get_cores user.org:user:flocra:1.0]]]]
set test_bench_file ${ip_path}/example_designs/bfm_design/flocra_v1_0_tb.sv
...
for { set i 1 } { 1 } { incr i } {
	set design_name "flocra_v1_0_bfm_${i}"
...
remove_files -quiet -fileset sim_1 flocra_v1_0_tb_include.vh
import_files -fileset sim_1 -norecurse -force $interface_address_vh_file
set_property top flocra_v1_0_tb [get_filesets sim_1]

...
marga_model::marga_model(int argc, char *argv[]) : MAX_SIM_TIME(200e6) {
	...
	+ argv0 + string(" csv=/path/to/test.csv\n\t")
	+ argv0 + string(" fst # (will dump to ") + argv0 + string(".fst)\n\t")
	...
	if (current_arg.substr(0, csvl).compare(csvstr) == 0) {
	    _csv_output = true;
	    if (current_arg.length() > csvl && current_arg[csvl] == '=') {
		filepath_csv = current_arg.substr(csvl + 1);
	    }
	    printf("Dumping CSV events to %s\n", filepath_csv.c_str());
	} else if (current_arg.substr(0, fstl).compare(fststr) == 0) {
	    _fst_output = true;
	    if (current_arg.length() > fstl && current_arg[fstl] == '=') {
		filepath_fst = current_arg.substr(fstl + 1);
	    }
	    printf("Dumping FST trace to %s\n", filepath_fst.c_str());
	} else if( current_arg.substr(0, trigl).compare(trigstr) == 0) {
	    _trig = true;
	    if (current_arg.length() > trigl && current_arg[trigl] == '=') {
